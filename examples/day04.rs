...
impl AoC for Day04 {
    ...
    fn part1(&self) -> Result<Self::PuzzleReturnType, &'static str> {
        let overlaps: Vec<&(LineSegment, LineSegment)> = self.segments.iter()
...
mod tests {
    ...
    fn test_part1() {
        assert_eq!(
            Day04::from_file(TEST_FNAME).unwrap().part1().unwrap(),
            2 
        );
    }
    ...
    fn test_part2() {
        assert_eq!(
            Day04::from_file(TEST_FNAME).unwrap().part2().unwrap(),
            4 
        );
    }
}
